...
void MQBDash::updateWithState(int speed,
  ...
  if (millis() - lastDashboardUpdateTime >= dashboardUpdateTime50) {
    ...
    sendTSK07();
    sendLhEPS01();
    sendMotor(rpm, coolantTemperature, oilTemperature);
    sendESP24();
    sendGear(gear);
    sendAirbag01();
...
void MQBDash::sendSteeringWheelControls(int button) {
  switch (button) {
    case 1: mfswBuf[0] = 0x08; mfswBuf[2] = 0x0F; break; // Menu (does nothing)
...
void MQBDash::sendMotor(int rpm, int coolantTemperature, int oilTemperature) {
  //
  // Motor_Code_01
  //
  ...
  //uint8_t mappedValoil = map(oilTemperature, -20, 190, 0x80, 0xE0);
  motor07Buf[2] = (60 + oilTemperature);
  //motor07Buf[2] = mappedValoil;
