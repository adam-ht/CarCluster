...
#include <ESPAsyncUDP.h>     // For game integration (install manually from: https://github.com/me-no-dev/ESPAsyncUDP )
...
#include <Arduino.h>
#include <ESPAsyncWiFiManager.h>  // For easier wifi management (install manually from: https://github.com/alanswx/ESPAsyncWiFiManager )
#include <ESPAsyncDNSServer.h>    // Requirement for ESPAsyncWiFiManager (install manually from: https://github.com/devyte/ESPAsyncDNSServer )
#include <ESPAsyncTCP.h>           // Requirement for ESP-DASH (install manually from: https://github.com/me-no-dev/ESPAsyncTCP )
...
#include <ESP8266WiFi.h>               // Arduino system library (part of ESP core)
...
const float speedCorrectionFactor = 1.090909090909091;   // Calibration of speed gauge
...
uint8_t backlight_brightness = 40;       // Backlight brightness 0-99 (only valid when backlight == true)
...
int oilTemperature = 60;                  // Oil Temperature 50-130C
...
Card oilTemperatureCard(&dashboard, SLIDER_CARD, "Oil Temperature", "C", 50, 190);
...
void setFuel(int percentage) {
  int fuel = map(percentage, 0, 100, minimumFuelPotValue, maximumFuelPotValue);
  //fuelPot.setPosition(desiredPosition, false);
}

void loop() {
  // Update the dashboard
  mqbDash.updateWithState(speed*speedCorrectionFactor, rpm*rpmCorrectionFactor, backlight_brightness, leftTurningIndicator, rightTurningIndicator, turning_lights_blinking, gear, coolantTemperature, oilTemperature, handbrake, highBeam, rearFogLight, doorOpen, outdoorTemperature, ignition);
  float fuelLevel = fuelQuantity;
  int fuelValue = map(fuelLevel, 0, 100, minimumFuelPotValue, maximumFuelPotValue);
...
void updateWebDashboard() {
  ...
  if (millis() - lastWebDashboardUpdateTime >= webDashboardUpdateInterval) {
    ...
    oilTemperatureCard.update(oilTemperature);
...
void listenForzaUDP(int port) {
  ...
  if (forzaUdp.listen(port)) {
    Serial.print("Forza UDP Listening on IP: ");
    Serial.println(WiFi.localIP());
...
void listenBeamNGUDP(int port) {
  ...
  if (beamUdp.listen(port)) {
    Serial.print("BeamNG UDP Listening on IP: ");
    Serial.println(WiFi.localIP());
