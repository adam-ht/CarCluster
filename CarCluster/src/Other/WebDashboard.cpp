...
WebDashboard::WebDashboard(GameState &game, int serverPort, unsigned long webDashboardUpdateInterval):
  ...
  oilTemperatureCard(&dashboard, SLIDER_CARD, "Oil temperature", "C", 50, 190),
  handbrakeCard(&dashboard, BUTTON_CARD, "Handbrake"),
  button1Card(&dashboard, BUTTON_CARD, "UP"),
  button2Card(&dashboard, BUTTON_CARD, "DOWN"),
  button3Card(&dashboard, BUTTON_CARD, "OK"),
  button4Card(&dashboard, BUTTON_CARD, "MENU"),
...
void WebDashboard::begin() {
  ...
  button1Card.attachCallback([&](int value) { //UP BUTTON
    button1Card.update(value);
    dashboard.sendUpdates();
    gameState.buttonEventToProcess = 1;
  });

  button2Card.attachCallback([&](int value) { //DOWN BUTTON
    button2Card.update(value);
    dashboard.sendUpdates();
    gameState.buttonEventToProcess = 2;
  });

  button3Card.attachCallback([&](int value) { //OK BUTTON
    button3Card.update(value);
    dashboard.sendUpdates();
    gameState.buttonEventToProcess = 3;
  });

  button4Card.attachCallback([&](int value) {  //MENU BUTTON
    button4Card.update(value);
    dashboard.sendUpdates();
    gameState.buttonEventToProcess = 4;
  });
