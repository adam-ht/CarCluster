...
#define RPM_CORRECTION_FACTOR 1.090909090909091

// Configure the maximum speed shown on the cluster (in km/h)
// Leave at 0 for using the defaults based on the cluster. Change this is your cluster has different limits
#define MAXIMUM_SPEED 240
...
#define MAXIMUM_COOLANT_TEMPERATURE 130
...
// specific configuration for my setup
#define buttonA 27
#define buttonB 25
#define buttonC 32
#define buttonD 12
...
#include "src/Libs/Bounce2/Bounce2.h" // For handling buttons
...
Bounce2::Button OK = Bounce2::Button();
Bounce2::Button MENU = Bounce2::Button();
...
void setup() {
  //Button setup
  UP.attach(buttonC,INPUT_PULLUP);
  UP.interval(5);
  UP.setPressedState( LOW ); 

  DOWN.attach(buttonA,INPUT_PULLUP);
  DOWN.interval(5);
  DOWN.setPressedState( LOW ); 

  MENU.attach(buttonD,INPUT_PULLUP);
  MENU.interval(5);
  MENU.setPressedState( LOW ); 

  OK.attach (buttonB,INPUT_PULLUP);
  OK.interval(5);
  OK.setPressedState( LOW );
...
void loop() {
  ...
  UP.update();
  if(UP.pressed()){
    SendSteeringWheelControls(4);
  }

  DOWN.update();
  if(DOWN.pressed()){
    SendSteeringWheelControls(5);
  }

  MENU.update();
  if(MENU.pressed()){
    SendSteeringWheelControls(7);
  }

  OK.update();
  if(OK.pressed()){
    SendSteeringWheelControls(6);
  }
...
void SendSteeringWheelControls(int button) {
  // Send the steering wheel controls to the cluster
  cluster.sendSteeringWheelControls(button);
}